---
description: "Generate or incrementally refresh coding-standards-drift-report.md for the active feature"
---
...
1. Load the existing `coding-standards-drift-report.md` if it exists. Parse every finding's identifier, title, severity, status, policy references, evidence, and description. Use it to preserve stable `CODE-STAND-DRIFT-###` identifiers for equivalent findings, preserve historical findings, and keep the correction-tracking link consistent. If the report still uses legacy `DRIFT-###` identifiers, migrate them to `CODE-STAND-DRIFT-###`.
...
7. Merge findings into the report:
   ...
   - Existing findings already marked `Resolved` stay in the report; reopen them to `Pending` only when the same drift is observed again.
8. Write `FEATURE_DIR/coding-standards-drift-report.md` with the merged finding history. Do not replace it with only the current review's findings.
...
**Remediation plan**: [Optional. Preserve when already present; `/speckit.coding-standards-drift-control.remediation-plan` adds or updates this for pending findings before generating tasks.]

**Resolution**: [For resolved findings only: concise note explaining why the finding is no longer pending.]
...
- Do not remove existing finding sections on rerun. Keep old findings intact, updating only identifier format, missing `Pending` status, reopened `Pending` status for observed drift, and clearly stale metadata that must change to remain accurate.
- Preserve existing finding `**Remediation plan**:` sections on rerun. Do not invent new remediation plans in this report command; remediation planning belongs to `/speckit.coding-standards-drift-control.remediation-plan`.
...
- If no drift is found and there are no historical findings, still write the report with `## Findings` stating that no coding-standards drift was identified in the reviewed scope.
